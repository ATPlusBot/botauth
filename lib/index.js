...
class BotAuthenticator {
    constructor(server, bot, options) {
        ...
        if (!this.options.scope) {
            throw new Error("options.scope can not be null");
        }
        ...
        this.server.get(`/${this.options.basePath}/:providerId`, this.options.resumption.persistHandler(), this.passport_redirect(this.options.scope));
        this.server.get(`/${this.options.basePath}/:providerId/callback`, this.passport_callback(this.options.scope), this.options.resumption.restoreHandler(), this.credential_callback());
        this.server.post(`/${this.options.basePath}/:providerId/callback`, this.passport_callback(this.options.scope), this.options.resumption.restoreHandler(), this.credential_callback());
    ...
    passport_redirect(scope) {
        let session = this.options.session;
        return (req, res, next) => {
            let providerId = req.params.providerId;
            return passport.authenticate(providerId, { session: session, scope: scope })(req, res, next);
        };
    }
    passport_callback(scope) {
        let session = this.options.session;
        return (req, res, next) => {
            let providerId = req.params.providerId;
            return passport.authenticate(providerId, { session: session, scope: scope})(req, res, next);
